...
import {executeHeaderSectionAction, headerSectionAction} from "./headerSectionAction";
...
export const HeaderSection = ({content}: HeaderSectionProps) => {
    ...
    const [state, setState] = useState({left:0, width:0, langageChanged: true, didMount: false});
    ...
    const customAnimation =  (target: Element) => {
        ...
        if(targetParent){
            if(!targetParent.classList.contains(stylesSass.active)){
                const list = nav.getElementsByClassName(stylesSass.active);
                for(let i = 0; i<list.length; i++){
                    list[i].classList.remove(stylesSass.active);
                }
                ...
                if(targetLeft>=currentLeft) {
                    nav.getElementsByClassName(stylesSass.line)[0].animate({
                ...
                targetParent.classList.add(stylesSass.active);
            }
        }
    }
    ...
    useEffect(() => {
        ...
        if(nav && state.langageChanged){ // after first render
            ...
            if(active.length) {
                ...
                setState({...state, left: active0.offsetLeft, width: active0.clientWidth, langageChanged: false});
            }
        }
    ...
    const handleClick = (e:  any) => {
        ...
        //     if(alist[i] == target){
        //         executeHeaderSectionAction(dispatch, headerSectionAction.Select, i);
        //         break;
        //     }
        // }
    }
    ...
    return (
      <>
          <section className={stylesSass.navbar}>
              <ul>
                  <li className={stylesSass.active}>
                      <a href="#first" onClick={((e) => handleClick(e))}>{t("header:section1")}</a></li>
                  <li><a href="#second" onClick={((e) => handleClick(e))}>{t("header:section2")}</a></li>
                  <li><a href="#third" onClick={((e) => handleClick(e))}>{t("header:section3")}</a></li>
                  <li><a href="#fourth" onClick={((e) => handleClick(e))}>{t("header:section4")}</a></li>
                  <li className={stylesSass.contactLi}><a href="#fifth" onClick={((e) => handleClick(e))}>{t("header:section5")}</a></li>
              </ul>
              {line}
          </section>
      </>
