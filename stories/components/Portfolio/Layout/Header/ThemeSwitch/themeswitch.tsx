...
export const ThemeSwitch = ({className = "", alignToRight = false}: ThemeSwitchProps) => {
    ...
    const CustomValueContainer = ({children, ...props}:  any) => {
        const {} = props;
        return(
            <ValueContainer {...props} className={styles.valueContainer} onBlur={HandleBlur}>
                <button onClick={toggleMenu} onTouchEnd={toggleMenu} onBlur={HandleBlur} autoFocus={true}>
                    {getOption(theme)?.icon}
                    <p className={[styles.buttonLabel, state.open ? styles.menuOpen : null].join(" ")}>
                        {getOption(theme)?.label}
                    </p>
                </button>
            </ValueContainer>)
    }
    ...
    const CustomMenu = ({children, ...props}: any) => {
        return(
            <Menu {...props} className={[styles.Menu, alignToRight ? styles.alignToRight : null].join(" ")}>
                <div>{children}
                </div>
            </Menu>)
    }
