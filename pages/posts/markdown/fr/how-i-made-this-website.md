...
date: '2022-09-21'
description: "Dans ce poste j'explique les différentes technologies que j'ai utilisées, comme **Next** et **React**, et les décisions que j'ai pris pour créer mon portfolio."
---

Vous souhaitez savoir comment j'ai créé mon site ? Vous êtes au bon endroit, dans ce poste, je vais détailler les différentes technologies que j'ai utilisées et pourquoi je les ai choisies.
...
Redux possède aussi plusieurs **dev-tools** permettant de vous aider à observer l'état de votre application, revenir en arrière, mais aussi à partager l'état de l'application d'un utilisateur qui aurait eu un bug non souhaité.
...
Je sais qu'il existe plusieurs alternatives à Redux plus simples à utiliser, cependant Redux est le plus populaire pour gérer les états des applications React et je désirais d'abord l'utiliser pour me faire ma propre opinion.
...
Dans un premier temps, j'écrivais mes animations à la main, en utilisant la library d'animations React par défaut. Puis j'ai découvert **Framer Motion** et ses animations **Layout**, qui animate automatiquement n'importe quel changement dans le DOM d'un composant donné, y compris un changement dans ses règles css.
...
Je n'ai pas grand-chose d'autres à dire en dehors de l'expérience développeur très satisfaisante. Je sais que Firebase devient cher lorsque le nombre d'utilisateurs actif devient intéressant, mais pour mon site web cela semblait suffisant.
...
De manière générale, vous êtes très limité sur ce que vous pouvez faire par rapport à blender, cependant, cela est idéal pour le web. Vous pouvez même réaliser un benchmark de votre scène pour savoir si elle ne risque pas de détruire les performances de votre site.
...
J'ai déjà eu l'opportunité de créer des chatbots, dans mes autres expériences de développeur web. Mais, le plus souvent on utilise un SDK existant et une app déjà construite, ce qui a un coût.
...
Le seul point négatif que je pourrais noter est que Medusa peut **manquer de fonctionnalités**, concernant la customisation de vos produits, ou encore le fait de devoir choisir une adresse de livraison, y compris si votre produit est digital. De plus j'aurais aimé pouvoir proposer des abonnements, pour passer commande automatiquement tous les x temps. 
...
Storybook permet de **tester vos components en isolation**, ainsi que de réaliser des tests de **régression visuelle**, pour savoir s'il y a eu une modification visuelle entre deux commits. De plus si vous utilisez **Figma** pour maquetter votre UI, il est très facile d'intégrer vos composants Figma dans Storybook.
...
Deux frameworks intéressants adoptant cette architecture sont [Astro]("https://astro.build/") et [Fresh]("https://fresh.deno.dev/"). En plus de ça, Astro vous permet d'utiliser **n'importe quel framework Front**, comme React Angular ou Vue, pour vous permettre de changer à tout moment, ou de travailler avec des développeurs qui n'utilisent pas forcément tous le même framework.
La deuxième chose dont je voudrais parler est qu'il aurait été une meilleure expérience de développeur pour moi si j'avais adopté l'architecture par microservices dès le début de mon développement. Cela signifie qu'il aurait été plus pratique de regrouper tous mes services au même endroit en utilisant **Docker ou Kubernetes**, plutôt que d'héberger mes services à plusieurs endroits avec Vercel, Heroku ou encore Railway... Cela aurait aussi permis de réduire le temps de communication entre chaque service.
...
J'aurais aussi commencé par mettre en place un serveur DHCP pour centraliser l'envoi de mail.
...
- [L'effet confetti]("https://codepen.io/Gthibaud/pen/ENzXbp") de Gthibaud, que j'ai utilisé pour la page commande validée.
- [FontAwesome]("https://fontawesome.com/"), tous mes icons viennent de leur version gratuite.
- [L'effet bordure magique]("https://codepen.io/gayane-gasparyan/pen/jOmaBQK") de Gayane, que j'ai utilisé pour les cartes de mes posts.
- [Le widget de chat]("https://codepen.io/coswise/pen/NWqNWqe") de Cosimo Scarpa, je n'ai pas utilisé son code, mais j'ai refait quelque chose de similaire avec Framer
- [Le bouton de confirmation]("https://codepen.io/coswise/pen/LYNaJrO") aussi de Cosimo Scarpa, que j'ai utilisé pour mon bouton de déconnexion sur ma page profile utilisateur.
- [Une animation de carte de crédit]("https://codepen.io/TurkAysenur/pen/wvaGqXW") de Aysenur Turk. Je l'ai utilisé pour mon bouton de validation de paiement par carte bleue.
- [Le blog de Maxime Heckel]("https://blog.maximeheckel.com/"), pour obtenir des informations sur Framer (en plus de la documentation officielle)
